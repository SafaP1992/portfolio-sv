...
{{--        <option value="{{$sub_cats->id}}">{{str_repeat('-----', $level)}} {{$sub_cats->title}}</option>--}}
        @if (count($sub_cats->childrenRecursive) > 0)
            @include('backend.partials.categories', ['categories' => $sub_cats->childrenRecursive, 'level' => $level+1, 'selected_category'=>$selected_category])
        @endif
    @endforeach
@else
    @foreach ($categories as $sub_cats)
        <option value="{{$sub_cats->id}}">{{str_repeat('-- ', $level)}}{{$sub_cats->title}}</option>
        @if (count($sub_cats->childrenRecursive) > 0)
            @include('admin.partials.categories', ['categories' => $sub_cats->childrenRecursive, 'level' => $level+1])
        @endif
    @endforeach
@endif
